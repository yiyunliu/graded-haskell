...
(* use ltac1 as the default in proof scripts *)
Set Default Proof Mode "Classic".

(* Wrapper for ltac1 simpl_env *)
Ltac2 simpl_env h0 := ltac1:(h |- simpl_env in h) (Ltac1.of_ident h0).

Ltac2 substitution_ih () :=
  lazy_match! goal with
  | [h3 : forall P4 P3 x0 phi0,
        [(?y, ?psi0)] ++ ?p2 ++ [(?x, ?phi)] ++ ?p1 = P3 ++ [(x0, phi0)] ++ P4 -> _
        |- _ ] =>
      let h3_constr := Control.hyp h3 in
      specialize ($h3_constr $p1 ([($y, $psi0)] ++ $p2) $x $phi ltac2:(auto) _ ltac2:(eauto));
      (* FFI call to ltac1 *)
      simpl_env h3;
      (* eauto from ltac2 seems buggy *)
      (* need to use the ltac1 eauto for the proof to go through *)
      rewrite subst_tm_tm_open_tm_wrt_tm in $h3; ltac1:(eauto using CGrade_lc);
      rewrite subst_tm_tm_var_neq in $h3
  end.

(* Use notation so you don't need to pass in the extra () *)
Ltac2 Notation "substitution_ih" := substitution_ih ().
...
Proof.
  ...
  all: try solve [simpl;
    ...
    repeat spec y;
    (* use ltac2 tactic when in classic proof mode *)
    ltac2:(substitution_ih);
    eauto].
  - (* Var *)
    destruct (x == x0).
